//window.onload = () => {
    ...
    async function demoPop(){
        ...
        if(Top==-1){
            ...
            ctx.fillText("element is tried to deleted called Stack underflow.",250,90); 
            ele2.disabled = false;
            ele.disabled=false
            topbtn.disabled=false
            isemptybtn.disabled=false
       ...
       /* if(cnt>0){
        ...
        ctx.fillText("So index "+(Top)+" value will be deleted from stack",250,50); 
    ...
    function demoNext(){
        switch(next){
            ...
            case 9: Stack6();
                ++next;
                break;
            case 10:lifoprinciple();
                 ++next;
                 break;
            case 11:basicoperation();
                 ++next;
                 break;
        }
    }
